...
# params
all_params = ll.get_all_params([encoder, decoder, generator], trainable=True)
...
all_grads = theano.grad(total_loss, wrt=all_params)
grads_norm = sum(T.sum(T.square(grad)) for grad in all_grads) / len(all_grads)

# updates
updates = optimizer_factory(params['optimizer'], all_grads, all_params, eta)
...
fn_output['k'] = k
fn_output['grads_norm'] = grads_norm
...
for epoch in range(3000000):
    ...
    for _ in range(params['epoch_iter']):
        ...
        loss_real_mean, loss_fake_mean, k_value, _, _ = model_out
